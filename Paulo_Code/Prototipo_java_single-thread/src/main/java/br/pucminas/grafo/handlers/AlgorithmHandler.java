 ...
 * <ul>
 * <li>ALGO_BFS → payload: graphId, source → result: List(Integer) ordem de
 * visita</li>
 * <li>ALGO_DFS → payload: graphId, source → result: List(Integer) ordem de
 * visita</li>
 * <li>ALGO_SHORTEST_PATH → payload: graphId, source, target → result:
 * Map{path,dist}</li>
 * <li>ALGO_TOPOLOGICAL_SORT → payload: graphId → result: List(Integer) ou null
 * se há ciclo</li>
 * <li>ALGO_STRONGLY_CONNECTED → payload: graphId → result: List(List(Integer))
 * SCCs (Kosaraju)</li>
 * </ul>
 */
public class AlgorithmHandler {
    ...
    private void onSCC(Event ev) {
        ...
        for (int i = 0; i < n; i++)
            if (!visited[i])
                fillOrder(g, i, visited, finishStack);
