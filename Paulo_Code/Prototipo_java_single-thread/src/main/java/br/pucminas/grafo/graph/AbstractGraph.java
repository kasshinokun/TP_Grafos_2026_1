 ...
 *
 * <p>
 * Implementa atributos compartilhados (rótulos e pesos de vértices)
 * e métodos auxiliares de validação de índice. As subclasses fornecem
 * a representação interna (matriz ou lista de adjacência).
 * </p>
 */
public abstract class AbstractGraph {
    ...
    protected final RepType repType; // para identificar a representação interna
    ...
    protected AbstractGraph(int numVertices, RepType repType) {
        ...
        this.vertexWeights = new double[numVertices]; // inicializa pesos com 1.0 por padrão
        this.vertexLabels = new String[numVertices]; // inicializa rótulos com "v0", "v1", ... por padrão
    ...
    // ── Validação ──────────────────────────────────────────────────────────
    // Garante que o grafo seja simples, evitando a criação de laço
    ...
    protected void checkEdge(int u, int v) {
        ...
        checkVertex(v);
        if (u == v)
            throw new IllegalArgumentException("Laço não permitido: " + u);
    }
