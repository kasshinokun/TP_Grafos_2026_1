package br.pucminas.grafo.graph.mining;
    ...
    public static List<Interaction> load(String path) throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                // ignora automaticamente auto-interações (uma pessoa comentando na própria
                // Issue). remove os laços indesejados para manter o grafo estritamente simples.
                if (actor.isEmpty() || target.isEmpty() || actor.equals(target))
                    continue;
                ...
                if (typeIdx >= 0 && parts.length > typeIdx) {
                    ...
                    try {
                        type = Interaction.InteractionType.valueOf(raw);
                    } catch (IllegalArgumentException e) {
                        System.err.println(
                                "[CSV] Tipo desconhecido '" + raw + "' na linha " + lineNo + ". Usando padrão.");
                    }
                }
    ...
    public static void generateSampleCsv(String path) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(path))) {
            ...
            for (int i = 0; i < 120; i++) {
                ...
                String target;
                do {
                    target = users[rnd.nextInt(users.length)];
                } while (target.equals(actor));
