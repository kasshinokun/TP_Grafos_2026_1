...
class AbstractGraph(ABC):
    ...
    def get_vertex_weight(self, v: int) -> float:
        return self.vertex_weights[v] if 0 <= v < self.num_vertices else 0.0
...
class AdjacencyMatrixGraph(AbstractGraph):
    def __init__(self, num_vertices: int):
        super().__init__(num_vertices)
        self.matrix = [[False for _ in range(num_vertices)] for _ in range(num_vertices)]
    ...
    def has_edge(self, u: int, v: int) -> bool:
        return 0 <= u < self.num_vertices and 0 <= v < self.num_vertices and self.matrix[u][v]

    def get_edge_count(self) -> int:
        return sum(row.count(True) for row in self.matrix)
