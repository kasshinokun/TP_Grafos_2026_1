# Interface Gráfica PyQt6 para o Projeto Delta v4c — REV D (PyQt6)
# Release: 2026-06-15
...
# Importações do projeto original (não modificadas)
...
QR_TEMP_FILE = os.path.join(TEMP_DIR, "qr_repository.png")


# ==============================================================================
# Classes auxiliares (parser GEXF e outros)
# ==============================================================================
class GraphValidateLoader:
    """Valida e carrega um grafo a partir de um arquivo .gexf usando parser XML manual (sem bibliotecas)."""
    @staticmethod
...
class MiningWidget(QWidget):
    ...
    def init_ui(self):
        ...
        sidebar_layout.addWidget(group_actions)
    ...
    def post_process(self):
        ...
        # Executa em thread separada para não travar
        import threading
        def run():
            try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Gera QR Code temporário do repositório
        self.qr_pixmap = self.generate_temp_qr()

        # Cria o stack de telas
        self.stack = QStackedWidget()
        self.mining_widget = MiningWidget()
        self.about_widget = AboutWidget(self.qr_pixmap)
        ...
        # Timer para atualizar o log (opcional, mas já temos sinais diretos)
        # No MiningWidget já usamos sinais para log/notificação, então não precisa.

    def generate_temp_qr(self) -> Optional[QPixmap]:
        """Gera o QR Code da URL do repositório e salva em arquivo temporário. Retorna QPixmap."""
        try:
            # Cria pasta temp se não existir
            os.makedirs(TEMP_DIR, exist_ok=True)
            # Gera a imagem usando o mesmo método do sistema original
            img = QRCodeJSONHandler.gerar_qr_code(REPO_URL, None)  # retorna PIL Image
            if img:
                img.save(QR_TEMP_FILE, "PNG")
                return QPixmap(QR_TEMP_FILE)
        except Exception as e:
            logging.warning(f"Falha ao gerar QR Code do repositório: {e}")
        return None

    def change_theme(self, theme: str):
        """Aplica tema claro/escuro/sistema (simulação com paletas)."""
        ...
        else:  # System – tenta detectar, mas deixa padrão
            app.setStyle("Fusion")
            app.setPalette(app.style().standardPalette())

    def closeEvent(self, event):
        """Ao fechar, remove o arquivo QR temporário."""
