# Interface Gráfica CustomTkinter para o Projeto Delta v4c — REV C (Sem dependências de grafos)
# Release: 2026-06-15
...
# ==============================================================================
# Classe para validação e carregamento de arquivos .gexf sem networkx
# ==============================================================================
class GraphValidateLoader:
    """Valida e carrega um grafo a partir de um arquivo .gexf usando XML nativo."""

    @staticmethod
    def validate(file_path: str) -> Optional[Dict[str, Any]]:
        """
        Retorna um dicionário com os dados do grafo:
        {
            'nodes': lista de ids dos nós,
            'edges': lista de tuplas (source, target),
            'node_count': int,
            'edge_count': int,
            'directed': bool
        }
        ...
        try:
            ...
            root = tree.getroot()
            # Namespace do GEXF (geralmente {http://www.gexf.net/1.2draft} ou similar)
            ns = {'ns': 'http://www.gexf.net/1.2draft'}
            # Se não encontrar, tenta sem namespace
            graph_tag = root.find('.//ns:graph', ns)
            if graph_tag is None:
                graph_tag = root.find('.//graph')
            if graph_tag is None:
                return None

            directed = graph_tag.get('defaultedgetype', 'undirected') == 'directed'

            nodes = []
            nodes_tag = graph_tag.find('.//ns:nodes', ns) if graph_tag.find('.//ns:nodes', ns) is not None else graph_tag.find('.//nodes')
            if nodes_tag is not None:
                for node in nodes_tag.findall('.//ns:node', ns) if nodes_tag.findall('.//ns:node', ns) else nodes_tag.findall('.//node'):
                    node_id = node.get('id')
                    if node_id:
                        nodes.append(node_id)

            edges = []
            edges_tag = graph_tag.find('.//ns:edges', ns) if graph_tag.find('.//ns:edges', ns) is not None else graph_tag.find('.//edges')
            if edges_tag is not None:
                for edge in edges_tag.findall('.//ns:edge', ns) if edges_tag.findall('.//ns:edge', ns) else edges_tag.findall('.//edge'):
...
class AboutFrame(ctk.CTkFrame):
    ...
    def _build_layout(self):
        ...
        try:
            qr_img = QRCodeJSONHandler.gerar_qr_code(REPO_URL, None)
            if qr_img:
            ...
            ctk.CTkLabel(self, text=f"Falha ao gerar QR Code:\n{e}", text_color="red").pack(pady=20)
