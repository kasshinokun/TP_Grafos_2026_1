 ...
 *   <li>METRIC_BRIDGING_TIES          → result: List(vertexIdx) pontes entre comunidades</li>
 * </ul>
...
public class MetricsHandler {
    ...
    private void onBetweenness(Event ev) {
        ...
        for (int s = 0; s < n; s++) {
            ...
            Arrays.fill(sigma, 0); Arrays.fill(dist, -1);
            sigma[s] = 1; dist[s] = 0;
            ...
            while (!queue.isEmpty()) {
                ...
                for (int w = 0; w < n; w++) {
                    ...
                    if (dist[w] < 0) { queue.add(w); dist[w] = dist[v] + 1; }
                    if (dist[w] == dist[v] + 1) { sigma[w] += sigma[v]; pred.get(w).add(v); }
                }
            }
        ...
        Map<Integer, Double> result = new LinkedHashMap<>();
        for (int v = 0; v < n; v++) result.put(v, cb[v] / norm);
        ev.setResult(result);
    }
    ...
    private void onCloseness(Event ev) {
        ...
        for (int src = 0; src < n; src++) {
            ...
            double sum = 0; int reachable = 0;
            for (int v = 0; v < n; v++)
                if (v != src && dist[v] < Double.MAX_VALUE) { sum += dist[v]; reachable++; }
        ...
        for (int it = 0; it < iter; it++) {
            ...
            for (int u = 0; u < n; u++) {
                ...
                if (out == 0) { // dangling node — distribui para todos
                    double share = pr[u] / n;
                    for (int v = 0; v < n; v++) nxt[v] += d * share;
                } else {
                    for (int v = 0; v < n; v++)
                        if (g.hasEdge(u, v)) nxt[v] += d * pr[u] / out;
                }
            }
            double diff = 0;
            for (int v = 0; v < n; v++) diff += Math.abs(nxt[v] - pr[v]);
            System.arraycopy(nxt, 0, pr, 0, n);
            if (diff < eps) break;
        }

        Map<Integer, Double> result = new LinkedHashMap<>();
        for (int v = 0; v < n; v++) result.put(v, pr[v]);
        ev.setResult(result);
    }
    ...
    private void onClustering(Event ev) {
        ...
        for (int v = 0; v < n; v++) {
            ...
            for (int u = 0; u < n; u++)
                if (g.hasEdge(v, u) || g.hasEdge(u, v)) neighbors.add(u);

            int k = neighbors.size();
            if (k < 2) { result.put(v, 0.0); continue; }

            int triangles = 0;
            for (int i = 0; i < k; i++)
                for (int j = i + 1; j < k; j++) {
                    int a = neighbors.get(i), b = neighbors.get(j);
                    if (g.hasEdge(a, b) || g.hasEdge(b, a)) triangles++;
                }
    ...
    private void onAssortativity(Event ev) {
        ...
        int m = edges.size();
        if (m == 0) { ev.setResult(0.0); return; }
        ...
        while (changed && maxIter-- > 0) {
            ...
            for (int idx : order) {
                ...
                int best = freq.entrySet().stream()
                               ...
                               .get().getKey();
                if (best != community[idx]) { community[idx] = best; changed = true; }
            }
        }
    ...
    private int[] shuffledOrder(int n) {
        ...
        for (int i = n - 1; i > 0; i--) { int j = rnd.nextInt(i + 1); int t = a[i]; a[i] = a[j]; a[j] = t; }
        return a;
    }
