 ...
 *
 * <p>Cada comando emite um {@link Event} ao {@link EventBus}, tornando
 * o CLI uma camada puramente de entrada/saída sobre a arquitetura EDA.</p>
 *
...
public class CLI {
    ...
    private boolean processCommand(String line) {
        ...
        try {
            switch (cmd) {
                ...
                case "betweenness"       -> cmdMetric(args, EventType.METRIC_BETWEENNESS_CENTRALITY, "Betweenness Centrality");
    ...
    private void cmdList() {
        ...
        if (ids.isEmpty()) { println("Nenhum grafo registrado."); return; }
    ...
    private void cmdAddEdge(String[] args) {
        requireArgs(args, 4);
        String id = args[1]; int u = Integer.parseInt(args[2]); int v = Integer.parseInt(args[3]);
    ...
    private void cmdHasEdge(String[] args) {
        ...
        Event ev = bus.publish(new Event(EventType.GRAPH_HAS_EDGE)
                .with("graphId", args[1]).with("u", Integer.parseInt(args[2])).with("v", Integer.parseInt(args[3])));
        if (ev.isSuccess()) println("hasEdge(" + args[2] + "," + args[3] + ") = " + ev.getResult());
        else println("[ERRO] " + ev.getErrorMessage());
    }

    private void cmdDegree(String[] args) {
        requireArgs(args, 3);
        String id = args[1]; int v = Integer.parseInt(args[2]);
    ...
    private void cmdShortest(String[] args) {
        ...
        if (!ev.isSuccess()) { println("[ERRO] " + ev.getErrorMessage()); return; }
    ...
    private void cmdTopSort(String[] args) {
        ...
        if (!ev.isSuccess()) { println("[ERRO] " + ev.getErrorMessage()); return; }
        List<?> r = ev.getResult();
        if (r == null) println("Ordenação topológica impossível — o grafo contém ciclos.");
        else println("Ordenação topológica: " + r);
    }
    ...
    private void cmdSCC(String[] args) {
        ...
        int i = 1;
        for (Object scc : sccs) println("  SCC " + i++ + ": " + scc);
    }
    ...
    private void cmdMetric(String[] args, EventType type, String label) {
        ...
        if (result.size() > 20) println("  ... (" + (result.size()-20) + " vértices omitidos)");
    }
    ...
    private void cmdBuildGraphs(String[] args) {
        ...
        if (!loadEv.isSuccess()) { println("[ERRO] " + loadEv.getErrorMessage()); return; }
    ...
    private void cmdShow(String[] args) {
        requireArgs(args, 2);
        AbstractGraph g = registry.get(args[1]);
        if (g instanceof AdjacencyMatrixGraph m) println(m.toMatrixString());
        else if (g instanceof AdjacencyListGraph l) println(l.toListString());
        else println(g.toString());
    }
    ...
    private void checkAndPrint(Event ev, String successMsg) {
        if (ev.isSuccess()) println(successMsg);
        else println("[ERRO] " + ev.getErrorMessage());
    }
