 ...
 *
 * <p>Cada método {@code testXxx} retorna {@code true} em sucesso
 * ou lança uma exceção / retorna {@code false} em falha.
 * O runner principal contabiliza aprovações e reprovações.</p>
 */
public class TestSuite {
    ...
    public static void main(String[] unused) throws Exception {
        ...
        for (String name : tests) {
            try {
                ...
                if (ok) { System.out.println("  ✓ " + name); pass++; }
                else    { System.out.println("  ✗ " + name + " [falhou]"); fail++; }
            } catch (Exception e) {
    ...
    private Application newApp() { return new Application(); }

    private Event pub(EventBus bus, Event ev) { return bus.publish(ev); }
    ...
    public boolean testAddAndRemoveEdge() {
        Application app = newApp(); EventBus bus = app.getBus();
    ...
    public boolean testEdgeWeights() {
        Application app = newApp(); EventBus bus = app.getBus();
        pub(bus, new Event(EventType.GRAPH_CREATE).with("graphId","g").with("numVertices",3).with("impl","list"));
        pub(bus, new Event(EventType.GRAPH_SET_EDGE_WEIGHT).with("graphId","g").with("u",0).with("v",2).with("weight",4.5));
    ...
    public boolean testShortestPath() {
        Application app = newApp(); EventBus bus = app.getBus();
        pub(bus, new Event(EventType.GRAPH_CREATE).with("graphId","g").with("numVertices",4).with("impl","list"));
        pub(bus, new Event(EventType.GRAPH_SET_EDGE_WEIGHT).with("graphId","g").with("u",0).with("v",1).with("weight",1.0));
        pub(bus, new Event(EventType.GRAPH_SET_EDGE_WEIGHT).with("graphId","g").with("u",1).with("v",3).with("weight",2.0));
        pub(bus, new Event(EventType.GRAPH_SET_EDGE_WEIGHT).with("graphId","g").with("u",0).with("v",2).with("weight",5.0));
        pub(bus, new Event(EventType.GRAPH_SET_EDGE_WEIGHT).with("graphId","g").with("u",2).with("v",3).with("weight",1.0));
    ...
    public boolean testCommunityDetection() {
        ...
        assert r.size() == 6;
        // vértices do mesmo clique devem ter mesma comunidade (não garantido sem pontes, mas verifica retorno)
        return true;
    }
