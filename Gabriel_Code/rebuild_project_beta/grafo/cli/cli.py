...
class CLI:
    def __init__(self, app):
        ...
        self.DEFAULT_NAMES_GRAPHS = ['graph1', 'graph2', 'graph3','graph_integrated']
    ...
    def process_command(self, line):
        ...
        try:
            ...
            elif cmd == "names-default" : return self.DEFAULT_NAMES_GRAPHS
    ...
    def print_help(self):
        ...
        print("  names-default                           — retorna lista com nomes padrão para os grafos")
    ...
    def cmd_g_exists(self,g_id:str)->bool:
        return self.registry.get(g_id)
...
#===========================================================================================================================v1
    def cmd_add_edge(self, args): # Exemplo: add-edge graph1 0 1 0.5
        if len(args) < 4: raise ValueError("Uso: add-edge <id> <u> <v> [peso]")
        gid, u, v = args[1], int(args[2]), int(args[3])
        
        # ✅ Verificação 1: Parse e valida peso ANTES de modificar o grafo
        w = None
        if len(args) > 4:
            try:
                w = float(args[4])
            except ValueError:
                raise ValueError("Peso inválido; use um número (ex: 0.5)")
        
        # ✅ Verificação 2: Auto-loop (grafo simples não permite ciclos de tamanho 1)
        if u == v:
            raise ValueError(f"Auto-loop não permitido: aresta {u} → {u} viola regras de grafo simples.")
        
        # ✅ Verificação 3: Duplicação (grafo simples não permite múltiplas arestas)
        ev_check = self.bus.publish(Event(EventType.GRAPH_HAS_EDGE).with_payload("graphId", gid).with_payload("u", u).with_payload("v", v))
        if ev_check.success and ev_check.result:
            raise ValueError(f"Aresta {u} → {v} já existe (grafo simples não permite múltiplas arestas).")
        
        # ✅ Verificação 4: Antiparalela (grafo simples não permite arestas em ambas direções)
        ev_anti = self.bus.publish(Event(EventType.GRAPH_HAS_EDGE).with_payload("graphId", gid).with_payload("u", v).with_payload("v", u))
        if ev_anti.success and ev_anti.result:
            raise ValueError(f"Aresta antiparalela não permitida: já existe {v} → {u}.")
        
        # ✅ Verificação 5: Adiciona aresta (só prossegue se passou em TODAS as validações acima)
        ev = self.bus.publish(Event(EventType.GRAPH_ADD_EDGE).with_payload("graphId", gid).with_payload("u", u).with_payload("v", v))
        
        # Validar sucesso antes de definir peso
        if not ev.success:
            raise ValueError(f"Erro ao adicionar aresta: {ev.error_message}")
        
        # ✅ Verificação 6: Define peso APENAS após sucesso na adição
        if w is not None:
...
#===========================================================================================================================
    def cmd_rem_edge(self, args): # Exemplo: rem-edge graph1 0 1
        if len(args) < 4: raise ValueError("Uso: rem-edge <id> <u> <v>")
        
        # Checagem se a aresta existe(criar metodo)
        gid, u, v = args[1], int(args[2]), int(args[3])
        
        # ✅ Verificação 2: Auto-loop (grafo simples não permite ciclos de tamanho 1)
        if u == v:
            raise ValueError(f"Auto-loop não permitido: aresta {u} → {u} desejada viola regras de grafo simples.")

        # ✅ Verificação 3: Verificação 
        ev_check = self.bus.publish(Event(EventType.GRAPH_HAS_EDGE).with_payload("graphId", gid).with_payload("u", u).with_payload("v", v))
        if ev_check.success and ev_check.result:
            ev_remove = self.bus.publish(Event(EventType.GRAPH_REMOVE_EDGE).with_payload("graphId", args[1]).with_payload("u", int(args[2])).with_payload("v", int(args[3])))
            self.check_and_print(ev_remove, "Aresta removida.")
        else:
            raise ValueError(f"NULL_EDGE_ERROR\nAresta desejada {u} → {v} não existe (foi removida anteriormente/não existe).")

    def cmd_has_edge(self, args): # Exemplo: has-edge graph1 0 1
    ...
    def cmd_degree(self, args): # Exemplo: degree graph1 0
        ...
        print(f"Vértice {v} — in-degree: {ev_in.result}, out-degree: {ev_out.result}") # Exemplo de saída: Vértice 0 — in-degree: 3, out-degree: 5
    ...
    def _print_list(self,lista:list):
        
        if len(lista) == 1:
            print(f"Grafo '{lista[0]}' construído.")
        else:
            # Junta todos menos o último com vírgula e aspas
            todos_menos_ultimo = ", ".join(f"'{item}'" for item in lista[:-1])
            ultimo = f"'{lista[-1]}'"
            print(f"Grafos {todos_menos_ultimo} e {ultimo} construídos.")
    ...
    def cmd_build_graphs(self, args):
        ...
        self._print_list(self.DEFAULT_NAMES_GRAPHS)
